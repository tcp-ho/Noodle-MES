    ...
    <div className="bg-white/70 backdrop-blur-xl rounded-2xl p-6 shadow-soft hover:shadow-lg transition-all duration-300 border border-white/60 relative overflow-hidden group h-full cursor-pointer">
        ...
        <div className="relative z-10 flex justify-between items-start">
            <div className="flex-1 min-w-0 flex flex-col gap-1">
                <p className="text-[11px] font-bold text-[#73A2BF] uppercase tracking-widest font-mono opacity-90 truncate">{title}</p>
...
export default function PurchaseRequisitionPage() {
    ...
    const getStatusBadgeClass = (s: string) => {
        const map: any = { 
            'Pending Verify': 'bg-[#F2C849]/10 text-[#B8860B] border-[#F2C849]/20', 
            'Pending Approve': 'bg-[#73A2BF]/10 text-[#73A2BF] border-[#73A2BF]/20', 
            'Approved': 'bg-[#448C11]/10 text-[#448C11] border-[#448C11]/20', 
            'Rejected': 'bg-[#F25D27]/10 text-[#F25D27] border-[#F25D27]/20',
            'Revise': 'bg-[#F25D27]/10 text-[#D66B70] border-[#F25D27]/20',
    ...
    const savePR = () => {
        ...
        setPrData(newData);
        closeModal();
        Swal.fire({ icon: 'success', title: 'Success', text: 'PR Saved Successfully', timer: 1500, showConfirmButton: false, confirmButtonColor: '#0D1826' });
    };
        ...
        <div className="flex flex-col h-full bg-[#D9D9D9]">
            ...
            <div className="px-4 py-2 flex justify-between items-center z-10 shrink-0 no-print">
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 flex items-center justify-center rounded-2xl bg-[#448C11] text-white shadow-lg shadow-[#448C11]/20 flex-shrink-0 border border-white/20">
                        <ShoppingCart size={24} />
                    </div>
                    <div>
                        <h1 className="text-2xl font-bold text-[#0D1826] tracking-tight whitespace-nowrap">PURCHASE REQUISITION</h1>
                        <p className="text-[#73A2BF] text-[10px] font-medium uppercase tracking-widest mt-0.5">ระบบขอซื้อวัตถุดิบและอุปกรณ์</p>
                    </div>
                </div>
                ...
                <div className="flex bg-white/40 p-1 rounded-md overflow-x-auto no-scrollbar max-w-full border border-white/50 shadow-inner w-fit flex-shrink-0 backdrop-blur-sm">
                    <button onClick={() => setActiveTab('kanban')} className={`px-4 py-1.5 rounded-sm text-xs font-bold transition-all flex items-center gap-2 whitespace-nowrap font-mono ${activeTab === 'kanban' ? 'bg-[#0D1826] text-[#F2C849] shadow-md' : 'text-[#73A2BF] hover:text-[#0D1826] hover:bg-white/50'}`}>
                        <Kanban size={14} /> BOARD VIEW
                    </button>
                    <button onClick={() => setActiveTab('log')} className={`px-4 py-1.5 rounded-sm text-xs font-bold transition-all flex items-center gap-2 whitespace-nowrap font-mono ${activeTab === 'log' ? 'bg-[#0D1826] text-[#F2C849] shadow-md' : 'text-[#73A2BF] hover:text-[#0D1826] hover:bg-white/50'}`}>
            ...
            <main className="flex-1 px-4 pb-4 pt-2 relative z-10 overflow-hidden flex flex-col gap-4 min-h-0">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 shrink-0">
                    <KpiCard title="Total PR" val={stats.total} color="#73A2BF" icon={FileText} desc="All Records" />
                    <KpiCard title="Pending Verify" val={stats.pendingVerify} color="#F2C849" icon={Search} desc="Purchaser Review" />
                    <KpiCard title="Pending Approve" val={stats.pendingApprove} color="#F25D27" icon={Clock} desc="Manager Approval" />
                    <KpiCard title="Approved" val={stats.approved} color="#448C11" icon={CheckCircle} desc="Ready for PO" />
                </div>
                ...
                <div className="bg-white/70 backdrop-blur-xl rounded-xl shadow-soft border border-white/60 flex flex-col overflow-hidden flex-1 min-h-0">
                                ...
                                <div className="flex items-center gap-1 p-1 bg-white/50 rounded-lg border border-white/50 backdrop-blur-sm shrink-0 shadow-inner">
                                    {['All', 'Pending Verify', 'Pending Approve', 'Revise', 'Approved', 'Rejected', 'Cancelled'].map(status => (
                                        <button key={status} onClick={() => setFilterStatus(status)} 
                                                className={`px-3 py-1.5 rounded-md text-[10px] font-bold transition-all whitespace-nowrap flex items-center gap-2 font-mono ${filterStatus === status ? 'bg-[#0D1826] text-[#F2C849] shadow-sm' : 'text-[#73A2BF] hover:text-[#448C11]'}`}>
                                            <span>{status}</span>
                                            <span className={`flex items-center justify-center h-4 min-w-[16px] px-1 rounded-full text-[9px] font-bold transition-all ${filterStatus === status ? 'bg-[#F2C849] text-[#0D1826]' : 'bg-gray-200 text-gray-500'}`}>{getStatusCount(status)}</span>
                                        </button>
                                    ))}
                                </div>
                                <div className="h-6 w-px bg-gray-200 mx-1 hidden xl:block shrink-0"></div>
                                <div className="relative w-full xl:w-64 shrink-0">
                                    <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#73A2BF]" />
                                    <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search PR..." className="w-full pl-9 pr-4 py-1.5 text-xs rounded-lg border border-gray-200/60 focus:outline-none focus:border-[#F2C849] bg-white/60 backdrop-blur-sm transition-colors font-mono" />
                                </div>
                            </div>
                            <div className="flex gap-2 shrink-0 flex-nowrap items-center ml-auto">
                                <button onClick={handleExcelImport} className="px-4 py-2 rounded-lg text-xs font-bold bg-white text-[#0D1826] border border-gray-200 hover:bg-gray-50 shadow-sm transition-all flex items-center gap-2 font-mono uppercase tracking-wide whitespace-nowrap">
                                ...
                                <button onClick={() => openModal('create')} className="px-4 py-2 rounded-lg text-xs font-bold bg-[#0D1826] text-[#F2C849] hover:bg-[#0D1826]/90 hover:shadow-lg shadow-md transition-all flex items-center gap-2 font-mono uppercase tracking-wide whitespace-nowrap">
                    ...
                    {activeTab === 'kanban' && (
                        <div className="flex-1 overflow-x-auto custom-scrollbar p-4 bg-gray-50/30">
                            <div className="flex gap-4 h-full min-w-max">
                                {/* Columns */}
                                {[
                                    { title: 'รอตรวจสอบ (Verify)', status: 'Pending Verify', color: '#F2C849' },
                                    { title: 'ส่งคืนแก้ไข (Revise)', status: 'Revise', color: '#F25D27' },
                                    { title: 'รออนุมัติ (Approve)', status: 'Pending Approve', color: '#73A2BF' },
                                    { title: 'อนุมัติแล้ว (Approved)', status: 'Approved', color: '#448C11' },
                                    { title: 'ยก���ลิก/ไม่อนุมัติ', status: 'Rejected', color: '#0D1826' }
                                ].map((col, idx) => (
                                    <div key={idx} className="w-80 flex-shrink-0 flex flex-col h-full bg-white/50 rounded-xl p-3 border border-white shadow-sm">
                                        <div className="flex justify-between items-center mb-3 px-1">
                                            <h4 className="font-bold text-[#0D1826] text-sm flex items-center gap-2"><div className="w-2 h-2 rounded-full" style={{backgroundColor: col.color}}></div> {col.title}</h4>
                                        ...
                                        <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-1">
                                            {getBoardItems(col.status).map(pr => (
                                                <div key={pr.id} className={`bg-white p-4 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-all cursor-pointer group relative ${col.status === 'Revise' ? 'border-l-4 border-l-[#F25D27]' : ''}`} onClick={() => openModal(col.status === 'Pending Verify' ? 'verify' : (col.status === 'Pending Approve' ? 'approve' : (col.status === 'Revise' ? 'edit' : 'view')), pr)}>
                                                    ...
                                                    <div className="mb-3">
                                                        <div className="text-xs font-bold text-[#0D1826]">{pr.department}</div>
                                                        <div className="text-[10px] text-[#73A2BF]">Req: {pr.requester}</div>
                                                        {col.status === 'Revise' && <div className="text-[10px] text-[#F25D27] bg-orange-50 p-1 rounded mt-1 italic line-clamp-2">Note: {getLastNote(pr)}</div>}
                                                    </div>
                                                    <div className="border-t border-gray-50 pt-2 flex justify-between items-center mt-2">
                                                        ...
                                                        <span className="text-xs font-bold text-[#0D1826]">{formatCurrency(pr.totalAmount)}</span>
                                                    </div>
                                                    
                                                    {/* Action Buttons based on status */}
                                                    <div className="mt-3">
                                                        {col.status === 'Pending Verify' && <button className="w-full bg-[#F2C849] text-[#0D1826] text-[10px] py-1.5 rounded hover:bg-yellow-500 transition-colors font-bold shadow-sm">Verify Request</button>}
                                                        {col.status === 'Revise' && <button className="w-full bg-[#F25D27] text-white text-[10px] py-1.5 rounded hover:bg-orange-700 transition-colors font-bold shadow-sm">Edit & Resubmit</button>}
                                                        {col.status === 'Pending Approve' && <button className="w-full bg-[#73A2BF] text-white text-[10px] py-1.5 rounded hover:bg-blue-600 transition-colors font-bold flex items-center justify-center gap-1 shadow-sm"><Stamp size={12}/> Approve</button>}
                                                        {col.status === 'Approved' && (
                                                            <div className="flex items-center gap-1 text-[#448C11] text-[10px] font-bold justify-center bg-green-50 py-1 rounded">
                        ...
                        <div className="flex-1 overflow-auto custom-scrollbar">
                            <table className="w-full text-left whitespace-nowrap">
                                <thead className="sticky top-0 z-10">
                                    <tr>
                                        <th className="px-4 py-3 text-[10px] font-bold text-[#73A2BF] uppercase bg-[#F2F0E4]/95 border-b-2 border-[#F2C849] font-mono">PR ID</th>
                                        <th className="px-4 py-3 text-[10px] font-bold text-[#73A2BF] uppercase bg-[#F2F0E4]/95 border-b-2 border-[#F2C849] font-mono">Date</th>
                                        <th className="px-4 py-3 text-[10px] font-bold text-[#73A2BF] uppercase bg-[#F2F0E4]/95 border-b-2 border-[#F2C849] font-mono">Department / Requestor</th>
                                        <th className="px-4 py-3 text-[10px] font-bold text-[#73A2BF] uppercase bg-[#F2F0E4]/95 border-b-2 border-[#F2C849] font-mono text-center">Items / Total Qty</th>
                                        <th className="px-4 py-3 text-[10px] font-bold text-[#73A2BF] uppercase bg-[#F2F0E4]/95 border-b-2 border-[#F2C849] font-mono text-right">Total Amount</th>
                                        <th className="px-4 py-3 text-[10px] font-bold text-[#73A2BF] uppercase bg-[#F2F0E4]/95 border-b-2 border-[#F2C849] font-mono text-center">Urgency</th>
                                        <th className="px-4 py-3 text-[10px] font-bold text-[#73A2BF] uppercase bg-[#F2F0E4]/95 border-b-2 border-[#F2C849] font-mono text-center">Status</th>
                                        <th className="px-4 py-3 text-[10px] font-bold text-[#73A2BF] uppercase bg-[#F2F0E4]/95 border-b-2 border-[#F2C849] font-mono text-center">Action</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-100/50">
                                    {paginatedData.map(pr => (
                                        <tr key={pr.id} className="hover:bg-[#F2C849]/5 transition-colors group bg-white/50">
                                            <td className="px-4 py-3 font-mono font-bold text-xs text-[#F2C849]">{pr.id}</td>
                                            <td className="px-4 py-3 text-xs text-gray-500 font-mono">{pr.date}</td>
                                            <td className="px-4 py-3">
                                                <div className="font-bold text-[#0D1826] text-xs">{pr.department}</div>
                                                <div className="text-[10px] text-[#73A2BF] mt-0.5 flex items-center gap-1"><User size={10}/> {pr.requester}</div>
                                            </td>
                                            <td className="px-4 py-3 text-center">
                                                <div className="font-bold text-[#0D1826] text-xs font-mono">{pr.items.length} List</div>
                                                <div className="text-[10px] text-[#73A2BF] font-mono mt-0.5">{pr.items.reduce((acc: number, i: any) => acc + (parseInt(i.qty) || 0), 0).toLocaleString()} Units</div>
                                            </td>
                                            <td className="px-4 py-3 text-right font-mono text-xs font-bold text-[#F25D27]">{formatCurrency(pr.totalAmount)}</td>
                                            <td className="px-4 py-3 text-center">
                                            ...
                                            <td className="px-4 py-3 text-center">
                                                <div className="flex justify-center gap-1">
                                                    {pr.status === 'Pending Verify' && (
                                                        <>
                                                            <button onClick={() => openModal('edit', pr)} className="w-7 h-7 flex items-center justify-center bg-white border border-gray-200 text-gray-400 hover:text-[#F2C849] hover:border-[#F2C849] hover:bg-gray-50 rounded shadow-sm transition-all"><Pencil size={14}/></button>
                                                    ...
                                                    {pr.status === 'Pending Approve' && (
                                                        <button onClick={() => openModal('approve', pr)} className="w-7 h-7 flex items-center justify-center bg-white border border-gray-200 text-blue-400 hover:text-white hover:bg-blue-500 hover:border-blue-500 rounded shadow-sm transition-all"><Stamp size={14}/></button>
                                                    )}
                                                    {pr.status === 'Approved' && (
                                                        <button onClick={() => handlePrint(pr.id)} className="w-7 h-7 flex items-center justify-center bg-white border border-gray-200 text-gray-400 hover:text-white hover:bg-[#0D1826] hover:border-[#0D1826] rounded shadow-sm transition-all"><Printer size={14}/></button>
                                                    )}
                                                    {['Revise', 'Cancelled', 'Rejected'].includes(pr.status) && (
                                                        <button onClick={() => openModal('view', pr)} className="w-7 h-7 flex items-center justify-center bg-white border border-gray-200 text-gray-400 hover:text-[#0D1826] hover:bg-gray-100 rounded shadow-sm transition-all"><Eye size={14}/></button>
                                                    )}
                                                </div>
                                            </td>
                                        </tr>
                    ...
                    {activeTab === 'log' && (
                        <div className="px-4 py-3 border-t border-gray-100/50 flex flex-col md:flex-row justify-between items-center gap-4 bg-white/40 backdrop-blur-sm shrink-0 rounded-b-xl">
                            <div className="flex items-center gap-2 text-xs text-[#73A2BF] font-bold font-mono">
                                <span>Show</span>
                                <select value={itemsPerPage} onChange={(e) => setItemsPerPage(Number(e.target.value))} className="bg-white/80 border border-gray-200/60 rounded-md px-2 py-1 focus:outline-none focus:border-[#F2C849] cursor-pointer">
                                    <option value={10}>10</option><option value={20}>20</option><option value={50}>50</option>
                                </select>
                                <span>entries</span>
                            </div>
                            <div className="flex items-center gap-2 font-mono">
                                <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-2 rounded-lg border border-gray-200/60 bg-white/80 hover:bg-white disabled:opacity-50 transition-colors text-[#73A2BF]"><ChevronLeft size={14}/></button>
                                <span className="text-xs font-bold text-[#0D1826] px-2 bg-white/80 border border-gray-200/60 py-2 rounded-lg">Page {currentPage} of {totalPages}</span>
                                <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="p-2 rounded-lg border border-gray-200/60 bg-white/80 hover:bg-white disabled:opacity-50 transition-colors text-[#73A2BF]"><ChevronRight size={14}/></button>
                            </div>
                        </div>
                    )}
                </div>
            ...
            {/* Modal */}
            {modalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0D1826]/60 backdrop-blur-sm animate-fade-in-up" onClick={closeModal}>
                    <div className="bg-white/95 backdrop-blur-xl w-full max-w-4xl max-h-[90vh] rounded-2xl shadow-2xl border border-white/50 flex flex-col overflow-hidden transform scale-100 transition-all" onClick={e => e.stopPropagation()}>
                        <div className="p-6 border-b border-gray-100 flex justify-between items-center shrink-0 bg-white">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 bg-[#0D1826] flex items-center justify-center rounded-xl shadow-lg text-[#F2C849]">
                                    <FilePlus size={24} />
                                </div>
                                <div>
                                    <h3 className="text-xl font-black text-[#0D1826] uppercase tracking-tight">
                                        {modalMode === 'create' ? 'Create New PR' : (modalMode === 'edit' ? 'Edit PR' : (modalMode === 'verify' ? 'Verify PR' : (modalMode === 'approve' ? 'Approve PR' : 'View PR')))}
                                    </h3>
                                    <div className="text-xs text-[#73A2BF] font-mono mt-1 flex items-center gap-2">
                            ...
                            <button onClick={closeModal} className="w-8 h-8 rounded-full hover:bg-gray-100 flex items-center justify-center text-gray-400 hover:text-[#0D1826] transition-all"><X size={20}/></button>
                        </div>

                        <div className="flex-1 overflow-y-auto custom-scrollbar p-8 bg-[#F9F8F4]">
                            {/* General Info */}
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative mb-6">
                                <h4 className="text-xs font-bold text-[#73A2BF] uppercase mb-4 tracking-widest border-b border-gray-100 pb-2">General Information</h4>
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    <div><label className="block text-[10px] font-bold text-[#73A2BF] uppercase mb-1.5 font-mono">Date</label><input type="date" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} readOnly={!['create', 'edit'].includes(modalMode)} className="w-full bg-transparent border-b border-gray-200 py-1 text-sm font-mono focus:outline-none focus:border-[#F2C849]" /></div>
                                    <div>
                                        <label className="block text-[10px] font-bold text-[#73A2BF] uppercase mb-1.5 font-mono">Department</label>
                                        <select value={formData.department} onChange={e => setFormData({...formData, department: e.target.value})} disabled={!['create', 'edit'].includes(modalMode)} className="w-full bg-transparent border-b border-gray-200 py-1 text-sm focus:outline-none focus:border-[#F2C849]">
                                    ...
                                    <div><label className="block text-[10px] font-bold text-[#73A2BF] uppercase mb-1.5 font-mono">Requester</label><input type="text" value={formData.requester} onChange={e => setFormData({...formData, requester: e.target.value})} readOnly={!['create', 'edit'].includes(modalMode)} className="w-full bg-transparent border-b border-gray-200 py-1 text-sm focus:outline-none focus:border-[#F2C849]" /></div>
                                    <div>
                                        <label className="block text-[10px] font-bold text-[#73A2BF] uppercase mb-1.5 font-mono">Urgency</label>
                                        <select value={formData.urgency} onChange={e => setFormData({...formData, urgency: e.target.value})} disabled={!['create', 'edit'].includes(modalMode)} className="w-full bg-transparent border-b border-gray-200 py-1 text-sm focus:outline-none focus:border-[#F2C849]">
                            ...
                            {/* Items */}
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative mb-6">
                                <h4 className="text-xs font-bold text-[#73A2BF] uppercase mb-4 tracking-widest border-b border-gray-100 pb-2">Items</h4>
                                {['create', 'edit'].includes(modalMode) && (
                                    <div className="flex gap-2 mb-4 items-end bg-gray-50 p-3 rounded-xl border border-gray-200">
                                        <div className="flex-1">
                                            <label className="block text-[10px] font-bold text-[#73A2BF] uppercase mb-1.5 font-mono">Select Item</label>
                                        ...
                                        <div className="w-24">
                                            <label className="block text-[10px] font-bold text-[#73A2BF] uppercase mb-1.5 font-mono">Qty</label>
                                        ...
                                        <button onClick={addItem} className="bg-[#0D1826] text-[#F2C849] p-2.5 rounded-lg font-bold text-xs shadow-md hover:bg-[#0D1826]/90 uppercase tracking-wide font-mono transition-transform active:scale-95">Add</button>
                                    </div>
                                )}
                                
                                <div className="overflow-hidden border border-gray-200 rounded-xl">
                                    <table className="w-full text-left whitespace-nowrap">
                                        <thead className="bg-gray-100 text-[10px] text-[#73A2BF] uppercase font-bold font-mono">
                                            <tr>
                                        ...
                                        <tbody className="divide-y divide-gray-100 text-xs">
                                            {formData.items.map((i: any, idx: number) => (
                                                <tr key={idx} className="hover:bg-gray-50">
                                                    <td className="p-3 font-mono font-bold text-[#0D1826]">{i.code}</td>
                                                    <td className="p-3 text-gray-600">{i.name}</td>
                                                    <td className="p-3 text-right font-mono font-bold text-[#0D1826]">{i.qty}</td>
                                                    <td className="p-3 text-right font-mono text-gray-500">{i.price.toLocaleString()}</td>
                                                    <td className="p-3 text-right font-mono font-bold text-[#F25D27]">{(i.qty * i.price).toLocaleString()}</td>
                                                    {['create', 'edit'].includes(modalMode) && (
                                        ...
                                        <tfoot className="bg-gray-50 border-t border-gray-200 font-mono text-sm">
                                            <tr>
                                                ...
                                                <td className="p-3 text-right font-bold text-[#0D1826]">฿{formData.items.reduce((sum: number, i: any) => sum + (i.price * i.qty), 0).toLocaleString()}</td>
                            ...
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative">
                                <h4 className="text-xs font-bold text-[#73A2BF] uppercase mb-4 tracking-widest border-b border-gray-100 pb-2 flex items-center gap-2">
                                    ...
                                    {formData.history && formData.history.length > 0 ? (
                                        formData.history.map((log: any, idx: number) => (
                                            <div key={idx} className="flex justify-between items-start text-xs border-b border-dashed border-gray-100 pb-2 last:border-0">
                                                <div>
                                                    <div className="font-bold text-[#0D1826] flex items-center gap-2">
                                                        <span className={`w-2 h-2 rounded-full ${log.action.includes('Approved') ? 'bg-[#448C11]' : (log.action.includes('Rejected') || log.action.includes('Cancelled') ? 'bg-[#F25D27]' : (log.action.includes('Revise') ? 'bg-[#F2C849]' : 'bg-[#73A2BF]'))}`}></span>
                                                        {log.action}
                                                    </div>
                                                    <div className="text-[10px] text-[#73A2BF] mt-0.5 font-mono">by {log.user}</div>
                        ...
                        <div className="p-6 border-t bg-gray-50 flex justify-end gap-3 shrink-0 z-10 rounded-b-2xl">
                             <button onClick={closeModal} className="px-6 py-2.5 text-[#73A2BF] hover:text-[#0D1826] text-xs font-bold hover:bg-gray-200 rounded-xl transition duration-300 uppercase tracking-widest font-mono">Cancel</button>
                             ...
                             {['create', 'edit'].includes(modalMode) && (
                                 <>
                                     ...
                                     <button onClick={savePR} className="px-8 py-2.5 bg-[#0D1826] hover:bg-[#0D1826]/90 text-[#F2C849] text-xs font-bold rounded-xl shadow-lg transition-all duration-300 transform active:scale-95 flex items-center gap-2 uppercase tracking-widest font-mono">
                             ...
                             {modalMode === 'verify' && (
                                 <>
                                     ...
                                     <button onClick={() => updateStatus('Pending Approve')} className="px-8 py-2.5 bg-[#F2C849] hover:bg-yellow-500 text-[#0D1826] text-xs font-bold rounded-xl shadow-lg transition-all duration-300 flex items-center gap-2 uppercase tracking-widest font-mono">
                             ...
                             {modalMode === 'approve' && (
                                 <>
                                     ...
                                     <button onClick={() => updateStatus('Approved')} className="px-8 py-2.5 bg-[#448C11] hover:bg-green-700 text-white text-xs font-bold rounded-xl shadow-lg transition-all duration-300 flex items-center gap-2 uppercase tracking-widest font-mono">
